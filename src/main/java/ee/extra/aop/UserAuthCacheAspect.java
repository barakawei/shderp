...
import ee.common.cache.BaseCacheAspect;
import ee.sys.auth.entity.Auth;
import ee.sys.auth.service.AuthService;
import ee.sys.group.entity.Group;
import ee.sys.group.entity.GroupRelation;
import ee.sys.group.service.GroupRelationService;
import ee.sys.group.service.GroupService;
...
import ee.sys.organization.service.OrganizationService;
import ee.sys.permission.entity.Permission;
import ee.sys.permission.entity.Role;
import ee.sys.permission.service.PermissionService;
import ee.sys.permission.service.RoleService;
import ee.sys.resource.entity.Resource;
 ...
 * <p/>
 * 1、当调用如下方法时，加缓存
 * {@link ee.sys.auth.service.UserAuthService#findRoles}
 * {@link ee.sys.auth.service.UserAuthService#findStringRoles}
 * {@link ee.sys.auth.service.UserAuthService#findStringPermissions}
 * <p/>
 * 2、授权（Auth）
 * 当增删改授权时，
 * 如果是用户相关的，只删用户的即可，
...
public class UserAuthCacheAspect extends BaseCacheAspect {
     ...
     * 其他的全部清理
     */
    @Pointcut(value = "target(ee.sys.auth.service.AuthService)")
     ...
     * 当删除资源时，清缓存
     */
    @Pointcut(value = "target(ee.sys.resource.service.ResourceService)")
     ...
     * 当删除权限时，清缓存
     */
    @Pointcut(value = "target(ee.sys.permission.service.PermissionService)")
     ...
     * 当修改角色show/role/resourcePermissions关系时，清缓存
     */
    @Pointcut(value = "target(ee.sys.permission.service.RoleService)")
     ...
     * 当删除/修改show字段时，清缓存
     */
    @Pointcut(value = "target(ee.sys.organization.service.OrganizationService)")
     ...
     * 当删除/修改show字段时，清缓存
     */
    @Pointcut(value = "target(ee.sys.organization.service.JobService)")
     ...
     * 当删除组时，清缓存
     */
    @Pointcut(value = "target(ee.sys.group.service.GroupService)")
     ...
     * 其他情况，清所有
     */
    @Pointcut(value = "target(ee.sys.group.service.GroupRelationService)")
     ...
     * 修改时，如果组织机构/工作职务变了，仅需清自己的缓存
     */
    @Pointcut(value = "target(ee.sys.user.service.UserService)")
    ...
    @Pointcut(value = "target(ee.sys.auth.service.UserAuthService)")
